...
class MeteoApi:
    ...
    def getDataforDb(self, date=None):
        self.col = self.conn.MeteoDb.MeteoData
        ver = self.col.find()
        test = list(ver)
        if len(test) == 0:
            print(0)
    ...
    def check_and_update(self):
        ...
        if self.db.name not in dbnames:
            ...
            lastDate=self.col.find().sort("date",-1).limit(1)[0]['date'].split("T")[0]
    ...
    def graph(self, nom, date,ch='tc'):
        ...
        self.Graph_dict={'pmer':["pression au miveau mer ",'Pa'],
            'tc':['temperateur','C°'],
                'ff':['vitesse de vente moyen 10 mn','m/s'],
                    'u':['humidite','%'],
